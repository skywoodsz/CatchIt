...
LeePositionControllerNode::LeePositionControllerNode(
  ...
  InitializeParams();
  //skywoodsz
  yaw_status = true; //true:关 false:开
  ...
  cmd_poly_sub_ = nh_.subscribe(
      "command/point", 1,
      &LeePositionControllerNode::PointCallback, this);
  ...
  odometry_sub_ = nh_.subscribe("/firefly/odometry_sensor1/odometry", 1,
                               &LeePositionControllerNode::OdometryCallback, this);
...
void LeePositionControllerNode::PointCallback(const geometry_msgs::PointConstPtr & point_msg)
{
  yaw_status = true;
}
